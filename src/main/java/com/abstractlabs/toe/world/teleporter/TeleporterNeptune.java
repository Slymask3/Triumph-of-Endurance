package com.abstractlabs.toe.world.teleporter;
...
public class TeleporterNeptune extends Teleporter {
	...
	public TeleporterNeptune(WorldServer worldserver) {
		super(worldserver);
			...
			for(int l = -2; l <= 2; ++l) {
				for(int i1 = -2; i1 <= 2; ++i1) {
					for(int j1 = -1; j < 3; ++j1) {
						...
						int i2 = k + i1 * b1 - l * b0;
						boolean flag = j1 < 0;
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? ToeBlocks.neptuneBrick : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double par2, double par4, double par6, float par8) {
		...
		if(this.destinationCoordinateCache.containsItem(j1)) {
			TeleporterNeptune.PortalPosition portalposition = (Teleporter.PortalPosition)this.destinationCoordinateCache.getValueByKey(j1);
			d3 = 0.0D;
			...
			for(l3 = l - short1; l3 <= 1 + 128; ++l3) {
				...
				for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for(int i2 = this.worldServerInstance.getActualHeight() -1; i2 >=0; --i2) {
						if(this.worldServerInstance.getBlock(l3, i2, l1) == ToeBlocks.portalNeptune) {
							while (this.worldServerInstance.getBlock(l3, i2 - 1, l1) == ToeBlocks.portalNeptune) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if(flag) {
				this.destinationCoordinateCache.add(j1, new TeleporterNeptune.PortalPosition(i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			if(this.worldServerInstance.getBlock(i - 1, j, k) == ToeBlocks.portalNeptune) {
				i4 = 2;
			}
			...
			if(this.worldServerInstance.getBlock(i, j, k - 1) == ToeBlocks.portalNeptune) {
				i4 = 3;
			}
            ...
            for (i3 = -1; i3 <= 1; ++i3) {
                for (j3 = 1; j3 < 3; ++j3) {
                    for (k3 = -1; k3 < 3; ++k3) {
                        ...
                        j4 = k2 + (j3 - 1) * l2 - i3 * l5;
                        flag = k3 < 0;
                        this.worldServerInstance.setBlock(l3, i4, j4, flag ? ToeBlocks.neptuneBrick : Blocks.air);
                    }
                }
            }
        }
        ...
        for (i3 = 0; i3 < 4; ++i3) {
            for (j3 = 0; j3 < 4; ++j3) {
                for (k3 = -1; k3 < 4; ++k3) {
                    ...
                    this.worldServerInstance.setBlock(l3, i4, j4, (Block)(flag ? ToeBlocks.neptuneBrick : ToeBlocks.portalNeptune), 0, 2); //create portal in dimension
                }
            }
