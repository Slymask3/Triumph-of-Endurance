...
public class ItemHasteTablet extends ItemToe
{
	private int lvl;

	public ItemHasteTablet(int lvl)
	{
		super();
		this.maxStackSize = 1;
		this.lvl = lvl;
	...
	public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
	{
		if (is.stackTagCompound != null)
		{
			if (is.stackTagCompound.getBoolean("activeAvailable"))
			{
				player.addPotionEffect(new PotionEffect(3, 5*20, 2));
				...
				LogHelper.info("onItemUse hasteTab");

				return true;
			}
			else 
			{
				Helper.msg(player, "Haste Tablet is on cooldown.", Color.red);
				return true;
			}
		}
		return false;
	...
	public void addInformation(ItemStack is, EntityPlayer player, List list, boolean par4)
	{
		if (is.stackTagCompound != null) 
		{
			...
			if (active) 
			{
				...
				list.add(EnumChatFormatting.GOLD + "given haste for 5 seconds.");
			} 
			else if(!active) 
			{
