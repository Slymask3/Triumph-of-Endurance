...
import com.abstractlabs.toe.network.PacketRecall;
...
public class GuiRecall extends GuiScreen {
	private EntityPlayer player;
	private TileEntityRecall tileEntity;
	private World world;
	private int x;
	private int y;
	private int z;
	...
	public GuiRecall(EntityPlayer player, TileEntityRecall e, World world, int x, int y, int z) {
		this.player = player;
		this.tileEntity = e;
		this.world = world;
		this.x = x;
	...
	public void drawScreen(final int par1, final int par2, final float par3) {
		...
		this.drawCenteredString(this.fontRendererObj, "Destination Block", this.width / 2, 20, 16777215);
        this.drawString(this.fontRendererObj, "Enter a unique name for this destination:", this.width / 2 - 4 - 150, 37, 10526880);
	...
	public void send() {
		tileEntity.setName(this.input.getText());
		Toe.packetPipeline.sendToServer(new PacketRecall(this.world, this.x, this.y, this.z, this.input.getText()));
		LogHelper.info("getText() == " + this.input.getText());
		
		Helper.msg(player, "Created a destination with the name of: " + this.input.getText(), Color.lime);
	}
}