...
public class ItemRenderAxepick implements IItemRenderer {
	...
	public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
		if(type == ItemRenderType.INVENTORY) {
			if(item.stackTagCompound == null) {
				IIcon icon = ((ItemToeAxepick) item.getItem()).getAxeIcon();
				GL11.glEnable(GL11.GL_BLEND);
	            renderItem.renderIcon(0, 0, icon, 16, 16);
			} else if(item.stackTagCompound != null && item.stackTagCompound.getBoolean("axe")) {
				IIcon icon = ((ItemToeAxepick) item.getItem()).getAxeIcon();
				GL11.glEnable(GL11.GL_BLEND);
	            renderItem.renderIcon(0, 0, icon, 16, 16);
			} else {
				IIcon icon = ((ItemToeAxepick) item.getItem()).getPickIcon();
				GL11.glEnable(GL11.GL_BLEND);
	            renderItem.renderIcon(0, 0, icon, 16, 16);
			}
		} else {
			if(item.stackTagCompound != null && item.stackTagCompound.getBoolean("axe")) {
				IIcon icon = ((ItemToeAxepick) item.getItem()).getAxeIcon();
