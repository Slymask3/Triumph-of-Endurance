...
public class SlayerHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "SlayerSkill";
	private static final String LEVEL = "SlayerLevel";
	private static final String EXP = "SlayerExp";
	private float exp;
	private int level;
	
	public SlayerHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new SlayerHelper(player));
	}
	
	public static SlayerHelper getProperties(EntityPlayer player) {
		return (SlayerHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new SlayerPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
