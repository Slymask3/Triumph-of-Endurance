package com.abstractlabs.toe.skill.runemaking;
...
public class RunemakingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "RunemakingSkill";
	private static final String LEVEL = "RunemakingLevel";
	private static final String EXP = "RunemakingExp";
	private float exp;
	private int level;
	
	public RunemakingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new RunemakingHelper(player));
	}
	
	public static RunemakingHelper getProperties(EntityPlayer player) {
		return (RunemakingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new RunemakingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
