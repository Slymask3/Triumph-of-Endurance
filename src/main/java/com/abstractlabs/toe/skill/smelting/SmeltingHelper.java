...
public class SmeltingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "SmeltingSkill";
	private static final String LEVEL = "SmeltingLevel";
	private static final String EXP = "SmeltingExp";
	private float exp;
	private int level;
	
	public SmeltingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new SmeltingHelper(player));
	}
	
	public static SmeltingHelper getProperties(EntityPlayer player) {
		return (SmeltingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new SmeltingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
