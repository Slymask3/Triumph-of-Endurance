...
public class FarmingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "FarmingSkill";
	private static final String LEVEL = "FarmingLevel";
	private static final String EXP = "FarmingExp";
	private float exp;
	private int level;
	
	public FarmingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new FarmingHelper(player));
	}
	
	public static FarmingHelper getProperties(EntityPlayer player) {
		return (FarmingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new FarmingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
