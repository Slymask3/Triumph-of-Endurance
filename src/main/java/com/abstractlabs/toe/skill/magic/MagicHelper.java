...
public class MagicHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "MagicSkill";
	private static final String LEVEL = "MagicLevel";
	private static final String EXP = "MagicExp";
	private float exp;
	private int level;
	
	public MagicHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new MagicHelper(player));
	}
	
	public static MagicHelper getProperties(EntityPlayer player) {
		return (MagicHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new MagicPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
