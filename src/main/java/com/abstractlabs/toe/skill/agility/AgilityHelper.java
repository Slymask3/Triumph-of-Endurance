...
public class AgilityHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "AgilitySkill";
	private static final String LEVEL = "AgilityLevel";
	private static final String EXP = "AgilityExp";
	private float exp;
	private int level;
	
	public AgilityHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new AgilityHelper(player));
	}
	
	public static AgilityHelper getProperties(EntityPlayer player) {
		return (AgilityHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new AgilityPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
