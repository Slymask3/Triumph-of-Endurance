...
public class MiningHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "MiningSkill";
	private static final String LEVEL = "MiningLevel";
	private static final String EXP = "MiningExp";
	private float exp;
	private int level;
	
	public MiningHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new MiningHelper(player));
	}
	
	public static MiningHelper getProperties(EntityPlayer player) {
		return (MiningHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new MiningPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
