...
public class CookingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "CookingSkill";
	private static final String LEVEL = "CookingLevel";
	private static final String EXP = "CookingExp";
	private float exp;
	private int level;
	
	public CookingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new CookingHelper(player));
	}
	
	public static CookingHelper getProperties(EntityPlayer player) {
		return (CookingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new CookingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
