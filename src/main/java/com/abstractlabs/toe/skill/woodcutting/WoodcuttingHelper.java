...
public class WoodcuttingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "WoodcuttingSkill";
	private static final String LEVEL = "WoodcuttingLevel";
	private static final String EXP = "WoodcuttingExp";
	private float exp;
	private int level;
	
	public WoodcuttingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new WoodcuttingHelper(player));
	}
	
	public static WoodcuttingHelper getProperties(EntityPlayer player) {
		return (WoodcuttingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new WoodcuttingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
