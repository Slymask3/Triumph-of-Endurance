...
public class PrayerHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "PrayerSkill";
	private static final String LEVEL = "PrayerLevel";
	private static final String EXP = "PrayerExp";
	private float exp;
	private int level;
	
	public PrayerHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new PrayerHelper(player));
	}
	
	public static PrayerHelper getProperties(EntityPlayer player) {
		return (PrayerHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new PrayerPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
