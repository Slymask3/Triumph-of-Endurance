...
public class BrewingHelper implements IExtendedEntityProperties {
	private final EntityPlayer player;
	private static final String NBT = "ToeSkills";
	private static final String SKILL = "BrewingSkill";
	private static final String LEVEL = "BrewingLevel";
	private static final String EXP = "BrewingExp";
	private float exp;
	private int level;
	
	public BrewingHelper(EntityPlayer player) {
		this.player = player;
		this.level = 1;
		this.exp = 0.0F;
	}
	...
	public static void addProperties(EntityPlayer player) {
		player.registerExtendedProperties(SKILL, new BrewingHelper(player));
	}
	
	public static BrewingHelper getProperties(EntityPlayer player) {
		return (BrewingHelper)player.getExtendedProperties(SKILL);
	}
	...
	public void setExperience(float i) {
		...
		if ((this.player instanceof EntityPlayerMP))
			ToePackets.network.sendTo(new BrewingPacket(this.level, progressPercentage()), (EntityPlayerMP)this.player);
	}
